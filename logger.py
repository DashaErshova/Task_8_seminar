...
def input_data():
    ...
    while var != 1 and var != 2:
        print('Некорректный номер варианта. Повторите ввод.')
...
def print_data():
    ...
    with open('Task_8_seminar\data_first_variant.csv', 'r', encoding='utf-8') as file:
...
def put_data():
    ...
    while number_file != 1 and number_file != 2:
        print('Некорректный номер файла. Повторите ввод.')
        number_file = int(input('Введите номер файла: '))
    ...
    if number_file == 1:
        ...
        if 0 <= number_journal < len(data_first):
            ...
            phone = phone_data()
            address = address_data()
            data_first[number_journal] = f'{surname}\n{name}\n{phone}\n{address}\n'
        ...
        if 0 <= number_journal < len(data_second):
            ...
            phone = phone_data()
            address = address_data()
            data_second[number_journal] = f'{surname};{name};{phone};{address}\n'
